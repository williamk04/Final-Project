...
static vehicle_status_t handle_server_response(const char *resp)
{
    ...
    cJSON *vehicle = cJSON_GetObjectItem(root, "vehicle");
    if (vehicle && cJSON_IsObject(vehicle))
    {
        cJSON *reserved = cJSON_GetObjectItem(vehicle, "reserved");
        if (reserved && cJSON_IsBool(reserved))
        {
            if (cJSON_IsTrue(reserved))
            {
                cJSON_Delete(root);
                return VEHICLE_RESERVED_OUT;
            }
            else
            {
                cJSON_Delete(root);
                return VEHICLE_NORMAL_OUT;
            }
        }
    }
